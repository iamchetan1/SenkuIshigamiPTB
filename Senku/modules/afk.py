...
from Senku.modules.redis.afk_redis import start_afk, end_afk, is_user_afk, afk_reason
from Senku import REDIS
from Senku.modules.users import get_user_id

from Senku.modules.helper_funcs.alternate import send_message
from Senku.modules.helper_funcs.readable_time import get_readable_time
...
def afk(update, context):
    ...
    try:
        senku = update.effective_message.reply_text(
...
def check_afk(update, context, user_id, fst_name, userc_id):
    if is_user_afk(user_id):
        ...
        if reason == "none":
            ...
            senku = update.effective_message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
            time.sleep(5)
            try:
                senku.delete()
            except BadRequest:
